// skyClouds.ts
// WebGL-based cloud renderer using 3D Simplex noise - OPTIMIZED VERSION
...
export class SkyCloudsRenderer {
  ...
  private createShaderProgram(): WebGLProgram {
    ...
    const fragmentShaderSource = `
      ...
      // Cloud density function with LOD support (optimized)
      float cloudDensity(vec3 p, float lod) {
        // Scale to make clouds more vertical (stretched in y) - configurable shape and size
        vec3 scaledP = p * vec3(2.0 * uSizeScale, uCloudConfig.y * uSizeScale, 2.0 * uSizeScale);
        
        float base = snoise((scaledP + vec3(uSeed)) * uCloudConfig.z);
        float detail1 = snoise((scaledP + vec3(uSeed * 1.7)) * uCloudConfig.z * 2.0) * 0.3;
        
        // Add higher frequency details only when LOD is high (close to camera)
        float detail2 = 0.0;
        float detail3 = 0.0;
        if (lod > 0.5) {
          detail2 = snoise((scaledP + vec3(uSeed * 3.1)) * uCloudConfig.z * 4.0) * 0.15;
        }
        if (lod > 0.7) {
          detail3 = snoise((scaledP + vec3(uSeed * 5.7)) * uCloudConfig.z * 8.0) * 0.075;
        }

        float d = base + detail1 + detail2 + detail3;
        d = smoothstep(0.1 + uCloudConfig2.x * 0.2, 0.6 + uCloudConfig2.x * 0.2, d);
        ...
        return d * uCloudConfig.x;
      }

      // Simplified density gradient normal (optimized with larger epsilon)
      vec3 densityNormal(vec3 p, float lod) {
        float eps = 0.1; // Increased from 0.05 for better performance
        float d = cloudDensity(p, lod);
        return normalize(vec3(
          cloudDensity(p + vec3(eps, 0.0, 0.0), lod) - d,
          cloudDensity(p + vec3(0.0, eps, 0.0), lod) - d,
          cloudDensity(p + vec3(0.0, 0.0, eps), lod) - d
        ));
      }

      // Ray marching function (optimized)
      vec4 marchClouds(vec3 ro, vec3 rd) {
        float t = 0.0;
        float alpha = 0.0;
        vec3 col = vec3(0.0);
        
        // Pre-calculate sun direction once
        vec3 sunDir = normalize(vec3(sin(uSunElevation * 3.14159 / 180.0), cos(uSunElevation * 3.14159 / 180.0), 0.0));
        
        // Pre-calculate wind transform once
        float windRadians = uWindDirection * 3.14159 / 180.0;
        float windX = cos(windRadians);
        float speed = uWindSpeed * 0.001;
        
        // Pre-calculate atmospheric response
        ...
        // Pre-calculate base cloud colors
        ...
        vec3 baseCloudColor = mix(nightCloudColor, dayDuskColor, duskToNight);
        float horizonInfluence = mix(nightHorizonInfluence, dayDuskHorizon, duskToNight);
        ...
        int maxSteps = 12; // Reduced from 16
        float stepSize = 0.15; // Slightly larger steps
        
        for (int i = 0; i < 12; i++) {
          // Early termination when alpha is saturated
          if (alpha > 0.95) break;
          
          vec3 pos = ro + rd * t;
          
          // Calculate LOD based on distance
          float lod = 1.0 - min(t / 1.5, 1.0);

          // Minimal turbulence for subtle variation
          vec3 turbulence = vec3(
            snoise(pos * 0.3 + uTime * 0.05) * 0.05,
            0.0,
            0.0
          );
          vec3 shapePos = pos + vec3(-uTime * speed * windX, 0.0, 0.0) + turbulence;
          float d = cloudDensity(shapePos, lod);

          // Skip expensive lighting calculations if density is negligible
          if (d < 0.01) {
            t += stepSize * 1.5; // Larger steps in empty space
            continue;
          }

          // Density-based lighting
          float densityLight = clamp(d * 1.2, 0.0, 1.0);

          // Proper directional lighting using density gradients (only when needed)
          vec3 n = densityNormal(pos, lod);
          float sunDot = clamp(dot(n, sunDir), 0.0, 1.0);

          // Optimized self-shadowing - adaptive samples based on density
          float shadow = 0.0;
          if (d > 0.1) { // Only calculate shadows for denser clouds
            vec3 shadowPos = pos;
            // Reduced from 4 to 2 shadow samples
            shadowPos += sunDir * 0.15;
            shadow += cloudDensity(shadowPos, lod * 0.7);
            shadowPos += sunDir * 0.15;
            shadow += cloudDensity(shadowPos, lod * 0.5);
            shadow = exp(-shadow * 1.2);
          } else {
            shadow = 1.0; // No shadow for thin clouds
          }
          ...
          float adjustedLight = pow(combinedLight, 1.0 / uCloudConfig3.y);
          vec3 cloudColor = mix(baseCloudColor * 0.7 * uCloudConfig3.x, baseCloudColor * uCloudConfig3.x, adjustedLight);

          // Add starlight tinting (subtle)
          float starlightContribution = starlightIntensity * (1.0 - combinedLight * 0.4);
          cloudColor = mix(cloudColor, starlightColor + baseCloudColor * 0.2, starlightContribution * smoothstep(0.2, 0.8, d));
          ...
          cloudColor = mix(vec3(cloudLuma), cloudColor, uCloudConfig3.z);
